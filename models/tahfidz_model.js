...
exports.getLast = (res, req) => {
    ...
    WHERE t.kode_user ="'+req.params.id+'" AND t.id_account="'+req.id_account+'" ORDER BY t.id_tahfidz DESC limit 1', function (err, rows) {
        if(err){
          return res.status(500).json({
              status: false,
              message: err.message,
...
exports.readDataListUser = (res, req) => {
    ...
    LEFT JOIN master_surah mm ON mm.no_surah = t.id_surah_from\
    LEFT JOIN master_surah mm2 ON mm2.no_surah = t.id_surah_to\
    AND t.tgl_tahfidz >= "'+start+'" \
    AND t.tgl_tahfidz <= "'+end+'" \
    WHERE t.kode_user ="'+req.kode_user+'" AND t.id_account="'+req.id_account+'" ORDER BY t.id_tahfidz DESC ', function (err, rows) {
        if(err){
          return res.status(500).json({
              status: false,
              message: err.message,
